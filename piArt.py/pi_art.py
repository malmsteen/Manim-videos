...
class PiCircle(Scene):
    ...
    def construct(self):
        ...
        path = VGroup(stroke_width=1, stroke_opacity=0.2)  # another combination is 1, 0.1
        paths = VGroup()  # group of bezier curves
        len_of_arc = 360 / num_of_arcs - offset / DEGREES # length of arc in DEGREESrees
        ...
        for i in range(n - 1):
            ...
            if pi_n[i] != pi_n[i + 1]:
                points = [bezier([p0, (p0+p1)*.2, p1])(t) for t in np.linspace(0, 1, 3)]
        ...
        self.play(Create(paths[3:]), *(paths[i].animate.set_stroke(width=1, opacity=.1) for i in range(3)), run_time=20, rate_func=linear)

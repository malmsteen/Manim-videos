...
class DoublePendulum(Scene):
    def construct(self):
        ...
        Center = Dot()
        Circle1 = Dot(
            radius=0.04*M1).move_to(x1[0]*RIGHT+y1[0]*UP).set_color(BLUE)
        Circle2 = Dot(
            radius=0.04*M2).move_to(x2[0]*RIGHT+y2[0]*UP).set_color(BLUE)
        ...
        for i in range(len(x1)-1):
            ...
            if i >= 30:
                for n in range(i, i-30, -1):
                    line1 = Line(x1[n]*RIGHT + y1[n]*UP, x1[n-1]*RIGHT + y1[n-1]
                                 * UP).set_stroke(YELLOW, width=2).set_opacity(0.03*a)
                    line2 = Line(x2[n]*RIGHT + y2[n]*UP, x2[n-1]*RIGHT + y2[n-1]
                                 * UP).set_stroke(BLUE, width=2).set_opacity(0.03*a)
                    traj.add(line1)
                    traj.add(line2)
                    a -= 1
                ...
                for n in range(i, 0, -1):
                    line1 = Line(x1[n]*RIGHT + y1[n]*UP, x1[n-1]*RIGHT + y1[n-1]
                                 * UP).set_stroke(YELLOW, width=2).set_opacity(0.05*a)
                    line2 = Line(x2[n]*RIGHT + y2[n]*UP, x2[n-1]*RIGHT + y2[n-1]
                                 * UP).set_stroke(BLUE, width=2).set_opacity(0.05*a)
                    traj.add(line1)
                    traj.add(line2)
                    a -= 1
